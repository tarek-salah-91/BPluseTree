...
public class NativeMethods<E extends Comparable<E>> {
    ...
    public int sameParent(Node<E> n, Node<E> parent, int size) {
        ...
        ArrayList<E> prev = null;
        if (n.getNext() != null) {
            next = n.getNext().getKeys();
            for (int i = 0; i < next.size(); i++) {
                if (keys.contains(next.get(i))) {
                    _next = true;
                    break;
                }
            }
        ...
        if (n.getPrev() != null) {
            prev = n.getPrev().getKeys();
            for (int i = 0; i < prev.size(); i++) {
                if (keys.contains(prev.get(i))) {
                    _prev = true;
                    break;
                }
            }
        }
        if (_next && next.size() - 1 >= Math.ceil(size / 2.0)) {
            return 1;
        } else if (_prev && prev.size() - 1 > Math.ceil(size / 2.0)) {
            return 2;
        } else {
            return 0;
        }
    ...
    public int nexOrprev(Node<E> n, Node<E> parent, int size) {
        ...
        if (_next && next.getKeys().size() - 1 >= 1) {
            return 1;
        } else if (_prev && prev.getKeys().size() - 1 >= 1) {
            return 2;
        } else {
            return 0;
        }
